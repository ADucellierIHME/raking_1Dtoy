...
for race in races:
    ...
    (result, lambda_k) = raking_entropic_distance(x_i, np.ones(len(x_i)), np.ones(len(x_i)), mu, 1.0, 500, False, True)
...
df_raked_cause['entropic_distance'] = df_raked_cause['entropic_distance'] / df_raked_cause['pop']
...
print('Entropic distance, rake by cause, diff = ', \
    np.sum(np.abs(df_raked['value_raked'].to_numpy() - df_raked_cause['entropic_distance'].to_numpy())))
...
df_raked_race['entropic_distance'] = df_raked_race['entropic_distance'] / df_raked_cause['pop']
df_raked_race.sort_values(by=['race', 'cause'], inplace=True)

# Compare values between two raking methods
print('Entropic distance, rake by race, diff = ', \
    np.sum(np.abs(df_raked['value_raked'].to_numpy() - df_raked_race['entropic_distance'].to_numpy())))
...
for race in races:
    ...
    if len(df_sub['all_cause_value'].unique()) != 1:
        print('The margin should be the same for all causes.')
    mu = df_sub['all_cause_value'].iloc[0]
    (result, lambda_k) = raking_l2_distance(x_i, np.ones(len(x_i)), np.ones(len(x_i)), mu, True)
    df_sub['value_raked'] = result
    df_raked.append(df_sub)
df_raked = pd.concat(df_raked)
df_raked['value_raked'] = df_raked['value_raked'] / df_raked['pop']
...
df_raked_cause['l2_distance'] = df_raked_cause['l2_distance'] / df_raked_cause['pop']
df_raked_cause.sort_values(by=['race', 'cause'], inplace=True)

# Compare values between two raking methods
print('L2 distance, rake by cause, diff = ', \
    np.sum(np.abs(df_raked['value_raked'].to_numpy() - df_raked_cause['l2_distance'].to_numpy())))
...
df_raked_race['l2_distance'] = df_raked_race['l2_distance'] / df_raked_race['pop']
df_raked_race.sort_values(by=['race', 'cause'], inplace=True)

# Compare values between two raking methods
print('L2 distance, rake by race, diff = ', \
    np.sum(np.abs(df_raked['value_raked'].to_numpy() - df_raked_race['l2_distance'].to_numpy())))
